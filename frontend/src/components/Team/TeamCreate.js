...
const TeamCreate = ({ getOpen, data }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await request('post', `create_message`, values);
  ...
  const handleSubmit2 = async () => {
    try {
      const response = await request('post', `create_take_action`, {
        // comments
        take_action_comments: comments
      });
      data(response.result);
      console.log(values);
      if (response.status === 'ok') {
    ...
    <form autoComplete="off" style={{ height: '100%' }}>
      ...
      <Card className={classes.cardContainer}>
        ...
        >
          ...
          <Button color="primary" variant="contained" onClick={handleSubmit2}>
            take
          </Button>
          {/* </Navigate> */}
          {/* </RouterLink> */}
